...
export default function DetalleSolicitudCard({solicitud, tipo}:Props) 
{
    ...
    const facultades  = useFacultades()//useStore(useFacultiesStore, (state) => state.data)
    const escuelas = useEscuelas()//useStore(useEscuelasStore, (state) => state.data)
    const { data: idiomas } = useSubjects()//useStore(useSubjectsStore, (state) => state.data)
