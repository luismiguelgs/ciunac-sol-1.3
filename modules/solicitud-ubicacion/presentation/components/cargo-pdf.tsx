  ...
  return (
    <Document>
      <Page size={A4_PAGE_SIZE} style={styles.page} wrap={false}>
        ...
        <Text style={styles.title}>CARGO PARA EXAMEN DE UBICACION</Text>
        <Text style={styles.text}>SE HA COMPLETADO EL PROCEDIMIENTO</Text>
        <Text style={styles.text}>{getText(texts, 'TEXTO_UBICACION_3')}</Text>
        <View style={styles.dataBlock}>
          <Text style={styles.data}>{`Tipo de solicitud: ${upper(solicitud.typeName)}`}</Text>
          ...
          <Text style={styles.data}>{`Pago: S/${solicitud.amount.toFixed(2)}`}</Text>
        ...
        <Text style={styles.text}>{getText(texts, 'TEXTO_UBICACION_4')}</Text>
      </Page>
    </Document>
  )
}
